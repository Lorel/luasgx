...
int main (int argc, char **argv) {
    ...
    if( sgx_init() < 0 ) return EXIT_FAILURE;
    luaL_requiref(L, "cjson", luaopen_cjson, 1);
    lua_pushcfunction(L, l_sgx_process);
    lua_setglobal(L, "sgxprocess");
    lua_pushcfunction(L, l_sgx_encrypt);
    lua_setglobal(L, "sgxencrypt");
    lua_pushcfunction(L, l_sgx_decrypt);
    lua_setglobal(L, "sgxdecrypt");
